...
function PlainLanguagePanel({ model, onAsk, selectedClauseId, onSelectClause }) {
  const expandedClause = selectedClauseId || model.clauses?.[0]?.id || '';
  return (
    <div className="panel-stack">
      <SectionHeading
      ...
      <div className="plain-layout">
        <aside className="clause-nav card" aria-label="Document sections">
          ...
          {(model.clauses || []).map((clause) => (
            <button className={`clause-nav-item ${expandedClause === clause.id ? 'active' : ''}`} type="button" key={clause.id} onClick={() => onSelectClause(clause.id)}>
...
export default function ClauseGuardApp() {
  ...
  const [context] = useState({ role: '', jurisdiction: '', objective: '' });
    ...
    <ErrorBoundary>
      ...
      <div className="app-shell">
        ...
        <main id="main-content" className="page-container">
          ...
          <div className="live-announcement" role="status" aria-live="polite">{isAnalyzing ? 'Analyzing document…' : notice}</div>
